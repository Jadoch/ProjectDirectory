...
$sqllist = "SELECT * FROM Banks WHERE Submit='yes'";
$banklistdata = mysql_query($sqllist,$accounts);
echo "<table border=1><tr>
<th>Company</th><th>District</th><th>City</th><th>Telephone</th><th>Email</th>
</tr>
";
while ($bankrecord = mysql_fetch_array($banklistdata)){
$val;
echo "<form method='post' action='list_bank.php'>";
echo "<tr>";
echo "<td>".$bankrecord['BankName']."</td>";
echo "<td>".$bankrecord["District"]."</td>";
echo "<td>".$bankrecord['City']."</td>";
echo "<td>".$bankrecord['Telephone']."</td>";
echo "<td>".$bankrecord['Email']."</td>";
echo "<td><a href=\"bank-details.php?whatever=".$bankrecord['Bank_id']."\" >Details</a></td>";
echo "</tr>";
echo "</form>";
}
